...
def savePos():
    ...
    for i in range(4):
        data = temp[i].split(":")
        for j in range(3): home[i].append(float(data[j+1]))
...
class UAV:
    ...
    RELATIVE_DISTANCE = [[0.0, 0.0, 0.0], [-6.126564, 0.0, 2.8793], [-8.4363810, -0.2, -4.3754125], [-2.699055, -0.1, -6.32067]]
    def anchorMatrix(self):
        ...
        if len(homelocation)==4:
            for i in range(4):
                for j in range(3):
                    self.RELATIVE_DISTANCE[i][j] = homelocation[i][j]-homelocation[0][j]
        print(self.RELATIVE_DISTANCE)

        for i in range(3):
            location=[]
            for j in range(3):
                location.append(homelocation[0][j]+self.RELATIVE_DISTANCE[i+1][j])
            homelocation.append([location[0],location[1],location[2]])
    ...
    def move(self,tar_pos,tar_angle,moveInfo=""):
        while True:
            self.region.setTarget(tar_pos)
            self.camera.setTarget(tar_angle)
            pos = self.region.reach()
            angle = self.camera.reach()

            arrive = self.comm.waitMove(pos, angle, moveInfo)
            if arrive:
                self.region.setCenter(pos)
                self.camera.setCenter(angle)
            if pos == tar_pos: break

    def getCurrentPos(self):
        pos = [[0.0], [0.0], [0.0], [0.0]]
        cur_pos = [1.0, 1.0, 1.0]
        cur_pos = self.comm.getPos()
        for i in range(3): pos[i][0] = cur_pos[i]
        pos[3][0] = 1.0
        pos = self.MATRIX.I * pos
        for i in range(3): cur_pos[i] = pos[i][0]
        return cur_pos
    ...
    def searchMode(self, args=None):  # abandon access to MODE
        ...
        while not self.stop_signal:
            ...
            self.region.setCenter(self.getCurrentPos())

            search_count = 0 # search more than twice, quit current search
            while self.MODE == SEARCH and search_count < 2:
                search_count += 1
                moveInfo = "UAV is approaching targeted position!"
                self.move(feature.region.tar_pos,feature.camera.angle,moveInfo)
                ...
                self.gui.imageQueue.append([ret.image, feature.name])  # final image is saved in the down part
                ...
                if ret.exist is True:
                    while self.MODE == SEARCH and ret.ideal is False and allcount<2:
                        ...
                        moveInfo =  "Search mode is adjusting position for feature " + feature.name
                        self.move(reachInfo[0],reachInfo[1],moveInfo)

                        # take photo and check ideal
                        image = self.comm.waitImage(COMPRESS)
                    ...
                    cv2.imwrite(feature.name+".jpg", image)
                    self.log.info("The situation is ideal. Further process is being executed.")
                    feature.postprocess(image, self)  # post process like storage or else
                    search_count = 0
            ...
            if search_count == 0:
                self.log.info("The feature is detected successfully!")
            else:
                self.log.info("Current feature detection failed!")
