...
class GUI(Frame):
    ...
    def initInterface(self, master=None):
        ...
        self.spMark = IntVar()
        checksp = Checkbutton(self.feature, text='Spot Feature', variable=self.spMark, anchor=NW)
        checksp.deselect()
        self.feature.add(checksp, width=UNIT_WID, height=HEAD_HEI)
    ...
    def insertLogInfo(self, string):
        ...
        # self.loginfo.set(self.info)
        with open(LOGINFO_PATH, 'w') as log_file:
    ...
    def showImage(self, image, name):
        ...
        if name == "center":
            self.show_image = ImageTk.PhotoImage(temp_image.resize((UNIT_WID*3, UNIT_HEI*2)))
            self.centerImage.config(image = self.show_image)
            return

        height *= 1.0; width *= 1.0
        if abs(height-width)>10.0:
            if UNIT_HEI / height < UNIT_WID / width:
                width *= UNIT_HEI / height * EMBED_FACTOR
                height = UNIT_HEI * EMBED_FACTOR
            else:
                width = UNIT_WID * EMBED_FACTOR
                height *= UNIT_WID / width * EMBED_FACTOR
